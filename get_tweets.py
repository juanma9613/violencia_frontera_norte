...
def format_tweets(tweets, user, _type, _query, geo_s):
  """Function that processes tweets to obtain specific information in a dataframe
    ...
    Args:
        ...
        geo_s (str): geocode "lat,long,radious"

    Returns:
        df_tw, df_rtw (tuple(pd.DataFrame)): dataframes with collected information
          from tweets and retweets respectively.
    """
  ...
  for idx, tweet in enumerate(tweets):
    ...
    if (_type
        == "mentions") and (tweet._json["user"]["screen_name"]
                            == user) and (tweet._json["in_reply_to_screen_name"]
                                          == user):
      pass
      ...
      if not (tweet._json["full_text"].startswith('RT @')):
        ...
        temp = [
            ...
            tweet._json["user"]["screen_name"], _query, False, None, _type,
            geo_s, tweet._json["geo"], tweet._json["user"]["location"],
...
def get_tweets_mentions(user,
  ...
  """get all mentions of a user, the query performed is the following to: <user> OR  @<user>
    ...
    Args:
        ...
        newest_id_possible_path (str, optional): filepath where we store the last
            search for each account Defaults to 'newest_id_mentions.json'.
...
def get_tweets_timeline(user,
  ...
  """Function to get tweets and retweets from an user timeline
    ...
    Args:
        ...
        api (tweepy.api.API): tweepy connection
        number_of_api_calls (int, optional): [description]. Defaults to 10.
        newest_id_possible_path (str, optional): filepath where we store the
            last id retrieved from twitter api for each user.
            Defaults to 'newest_id_tweets.json'.
  ...
  while (n_queries < max_queries):
    ...
    if (tweet_max_id is not None) and (_since_id is not None):
      if tweet_max_id < _since_id:
        print('exited because there arent more tweets to collect')
        break
...
def create_arg_parser():
  ...
  parser.add_argument("-U",
                      ...
                      default="users.txt",
                      type=str,
                      help="path to a txt file where each row is an user")
  return parser
...
if __name__ == "__main__":
  ...
  number_of_api_calls_per_user = 10
  ...
  # reading users from txt file
  users_path = args.users_path
  assert os.path.exists(
      users_path), "you didn't add a valid path to read users from"
  users = load_users(users_path)
  print('users', users)

  assert len(users) > 0, "you need to provide a txt file with at least one user"

  assert len(users) < MAX_CALLS_QUERY_API, "please reduce the number of users"

  if number_of_api_calls_per_user * len(users) > MAX_CALLS_QUERY_API:
    number_of_api_calls_per_user = MAX_CALLS_QUERY_API // len(users)

  # getting an instance to make api calls to twitter
  ...
  for i in users:
    ...
    # Getting mentions
    #  ---------------
    tw_m, rtw_m = get_tweets_mentions(
        _user,
        api=api,
        number_of_api_calls=number_of_api_calls_per_user,
        items_per_call=100)
    print((len(tw_m) + len(rtw_m)), "mentions collected for user: ", i)

    # Getting tweets and retweets from user timeline
    # ----------------------------------------------
    tw, rtw = get_tweets_timeline(
        _user,
        api=api,
        number_of_api_calls=number_of_api_calls_per_user,
        items_per_call=200)
    print(len(tw), "tweets", "and", len(rtw), "retweets collected for user: ",
          i)
  ...
  # quoting=csv.QUOTE_ALL helps to preserve the original tweet in case that it
  # contains either commas or tabs
